...
def _worker(args):
    wl, theta = args
    res = multipole_at(wl, theta, 0.0, 0.0, nh=NH, disc=DISC, nz=NZ)
...
if __name__ == "__main__":
    ...
    np.savez(
        os.path.join(OUTDIR, "map_kx_multipole.npz"),
        wavelength_nm=WLS,
        angle_deg=ANGLES,
        R=R.reshape(shape),
    ...
    print(f"done in {time.time()-t0:.0f}s -> map_kx_multipole.npz", flush=True)
